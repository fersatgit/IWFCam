...
type
  ...
  TTEGHeader =packed record
              Signature: array[0..3] of AnsiChar;       //0TEG or 1TEG or 2TEG
              Cmd:       word;
              DataLen:   word;
              end;
  PTEGHeader=^TTEGHeader;
  TUDPInfo   =packed record
              Unknown:        array[0..17] of byte;
              UDPPort:        word;
              IPAddr:         dword;
              end;
  TGetUDPInfo=packed record                             //1TEG cmd=$0C
              TEGHeader: TTEGHeader;
              Unknown1:  dword;
              Unknown2:  dword;
              UDPInfo:   TUDPInfo;
              CamName:   array[0..26] of AnsiChar;
              Unknown3:  int64;
              end;
  PGetUDPInfo=^TGetUDPInfo;
  TSetUDPInfo=packed record                             //1TEG cmd=$0D
              TEGHeader:      TTEGHeader;
              UDPInfo:        TUDPInfo;
              Unknown2:       byte;                     //default - 9
              PasswordHash:   array[0..11] of AnsiChar; //default - "9e8040834b3a"
              Align:          array[0..2] of byte;
              end;
  TImageData =packed record                             //2TEG cmd=$03
              TEGHeader:    TTEGHeader;
              Unknown:      int64;
              FrameIndex:   word;
              PacketIndex:  word;
              ImageDataLen: dword;
              MixedData:    array[0..65535] of byte; //ImageData started from TegHeader.DataLen-16-ImageDataLen
              end;
  ...
  MIB_IPADDRTABLE=packed record
                dwNumEntries: dword;
                table:        array[0..65535] of MIB_IPADDRROW;
                end;
  PMIB_IPADDRTABLE=^MIB_IPADDRTABLE;

function GetBestInterface(dwDestAddr: dword;pdwBestIfIndex: pdword):dword;stdcall;external 'Iphlpapi.dll';
function GetIpAddrTable(pIpAddrTable: PMIB_IPADDRTABLE;pdwSize: pdword;bOrder: boolean):dword;stdcall;external 'Iphlpapi.dll';

const
  CamIPAddr                  =$010AA8C0; //192.168.10.1
  Width                      =640;
  Height                     =480;
...
const
  GetUDPInfo:                 array[0..19] of byte=($31,$54,$45,$47,$0b,$00,$0c,$00,$01,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00);
  SetUDPInfo:                 TSetUDPInfo=(TEGHeader:(Signature:'1TEG';Cmd:$0D;DataLen:40);Unknown2:9;PasswordHash:'9e8040834b3a');
  GetStream:                  array[0..35] of byte=($32,$54,$45,$47,$01,$00,$1c,$00,$01,$00,$00,$00,$00,$00,$00,$00,$01,$00,$00,$00,$08,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$01,$00,$00,$00);
var
  i,len:                          integer;
  ...
  tmp:                            pointer;
  IpTable:                        MIB_IPADDRTABLE absolute buf;
begin
  ...
  repeat
    ...
    begin
      ...
      repeat
        ...
        sock_addr.sin_addr.S_addr:=CamIPAddr;
      ...
      len:=sizeof(Buf);
      GetIpAddrTable(@IpTable,@len,false);
      GetBestInterface(CamIPAddr,@len);
      for i:=0 to IpTable.dwNumEntries-1 do
        with IpTable.table[i] do
          if (dwIndex=len)and((dwAddr xor CamIPAddr) and dwMask=0) then
          begin
            SetUDPInfo.UDPInfo.IPAddr:=dwAddr;
            sock_addr.sin_addr.S_addr:=dwAddr;
            break;
          end;

      sock_addr.sin_port:=swap(41773);
      Sock:=socket(AF_INET,SOCK_DGRAM,IPPROTO_UDP);
      while bind(Sock,sock_addr,sizeof(sock_addr))<>0 do
        inc(sock_addr.sin_port);
      i:=500;
      setsockopt(Sock,SOL_SOCKET,SO_RCVTIMEO,@i,4);

      SetUDPInfo.UDPInfo.UDPPort:=sock_addr.sin_port;
      sock_addr.sin_addr.S_addr :=CamIPAddr;
      sock_addr.sin_port        :=swap(10104);
      SendTo(Sock,GetUDPInfo,sizeof(GetUDPInfo),0,sock_addr,sizeof(sock_addr));
      Recv(Sock,buf,sizeof(Buf),0);
      sock_addr.sin_port:=PGetUDPInfo(@Buf)^.UDPInfo.UDPPort;
      SendTo(Sock,SetUDPInfo,sizeof(SetUDPInfo),0,sock_addr,sizeof(sock_addr));
      SendTo(Sock,GetStream,sizeof(GetStream),0,sock_addr,sizeof(sock_addr));

      i:=0;
      LastHeartBeatTime:=icmp_request.seq_num;
    end
    else if (PTEGHeader(@buf)^.Signature='2TEG')and(PTEGHeader(@buf)^.Cmd=3) then
      with PImageData(@buf)^ do
      begin
        if (PacketIndex=0)and(pword(@BackBuf[0])^=$D8FF)and(pword(@BackBuf[i-2])^=$D9FF) then //Check SOI and EOI JPEG markers to ensure corect data received
        begin
      ...
      move(MixedData[TEGHeader.DataLen-16-ImageDataLen],BackBuf[i],ImageDataLen);
      inc(i,ImageDataLen);
    end;

    CurrentTime:=GetTickCount;
    if CurrentTime-LastHeartBeatTime>5000 then   //Send heartbeat cmd every 2 seconds
    begin
      SendTo(Sock,GetStream,sizeof(GetStream),0,sock_addr,sizeof(sock_addr));
